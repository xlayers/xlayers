...
export class CssRenderService {
  constructor(private webContext: WebContextService) {}
  ...
  private buildAstStyleSheet(styles: StyleList[], ast: SketchMSLayer): void {
    const computeStyle = (_ast: SketchMSLayer, _styles) => {
      const content = (name: string, data: string[]) => {
        if (data) {
          styles.push({ className: name, declarations: data });
        }
      };
      if (this.webContext.identify(_ast)) {
        _ast.layers.forEach(layer => {
          const rules: string[] = [];
          // tslint:disable-next-line:forin
          (layer as any).web.css.forEach(value => {
            rules.push(value);
          });
          content(`${(layer as any).web.className}`, rules);
          computeStyle(layer, [
            {
              className: `${(layer as any).web.className}`,
              declarations: rules
            }
          ]);
        });
