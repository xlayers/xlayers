describe('Editor page', () => {
  ...
  it('when loading a design file, the left sidebar should render and shows the correct layers structure', () => {
    ...
    cy.get('xly-tree-layout mat-tree')
      .children()
      .then((welcomeBackTree) => {
    ...
    cy.get(
      ".mat-tree > [data-id='E7B1B481-EA1B-449A-A8A1-75F4E84C7B9D']"
    ).click({ force: true });
    cy.get(
      ".mat-tree > [data-id='3FE0C2A6-A817-47D9-B070-ED2CD9D39D7D']"
    ).contains('YES');
    cy.get(
      ".mat-tree > [data-id='A147FF7C-F32C-4E80-B23B-2708FF7B1880']"
    ).click({ force: true });
    cy.get(
      ".mat-tree > [data-id='B40DD28B-85D6-4D87-A801-0E543573111E']"
    ).contains('NO');
  });

  it('when selecting a layer in the left sidebar, the design should highlight the correct layer in the canvas are', () => {
    cy.get('xly-tree-layout').contains('Welcome back').click({ force: true });
    cy.get(
      ".mat-tree > [data-id='A147FF7C-F32C-4E80-B23B-2708FF7B1880']"
    ).click({ force: true });
    cy.get(
  ...
  it('when selecting a layer of a design file, the left sidebar should highlight the correct layer', () => {
    cy.get('xly-viewer-canvas').then((subject) => {
      ...
      cy.wrap(subject).contains('YES').click({ force: true });
    });
    cy.get('mat-tree-node.selected').contains('YES');

    cy.get('xly-viewer-canvas').then((subject) => {
      cy.wrap(subject).click({ force: true });
      cy.wrap(subject).contains('NO').click({ force: true });
    });
  ...
  it('when selecting a layer of a design file, the right sidebar should show up', () => {
    ...
    cy.get('xly-viewer-layer').first().click({ force: true });
  ...
  context('using toolbar zoom', () => {
    it('when using the toolbar buttons + zoom the design canvas should react accordingly', () => {
      cy.get('[data-cy=zoomIn]').click({ force: true });
    ...
    it('when using the toolbar buttons - zoom the design canvas should react accordingly', () => {
      cy.get('[data-cy=zoomOut]').click({ force: true });
    ...
    it('when using the toolbar buttons reset zoom the design canvas should react accordingly', () => {
      ...
      cy.get('[data-cy="zoomReset"]').click({ force: true });
      cy.get('xly-viewer-canvas > div').then((canvas) => {
  ...
  it('when using the toolbar buttons show layer the design canvas should react accordingly', () => {
    cy.get('[data-cy=togglePreviewMode]').click({ force: true });
    cy.get('[data-cy=togglePreviewMode]').then((button) => {
      expect(button).to.have.attr('ng-reflect-color', 'warn');
    });
    cy.get('xly-viewer-layer')
      .first()
      .then((canvas) => {
  ...
  it(
    ...
    { browser: '!electron' },
    () => {
      cy.get('[data-cy=toggle3D]').click({ force: true });
