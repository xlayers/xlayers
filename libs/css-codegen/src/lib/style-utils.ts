...
function getCleanNumber(number: any) {
  if (!isNaN(number)) {
    if (number.includes('e')) {
      ...
      // In .sketch files' conversion, values that have exponential format on the generated CSS (including the 'e'),
      // are displayed on the editor with a 0 value, they are always position-related values (top, left, right, bottom).
